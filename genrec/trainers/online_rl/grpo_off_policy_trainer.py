...
class OffPolicyTrainer(BaseOnlineRLTrainer):
    ...
    def _generate_and_score_completions(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if num_generated > 0:
            ...
            # 提取 beam search 的序列分数（length-normalized log prob）
            # 安全访问 sequences_scores
            if hasattr(outputs, 'sequences_scores') and outputs.sequences_scores is not None:
                generated_scores = outputs.sequences_scores  # [total_batch_size * num_generated]
            else:
                # 如果 sequences_scores 不存在，尝试从 scores 手动计算
                # 先打印调试信息
                print(f"WARNING: sequences_scores not found. Available attributes: {dir(outputs)}")
                if hasattr(outputs, 'scores') and outputs.scores is not None and len(outputs.scores) > 0:
                    # 从 scores 手动计算 length-normalized log probabilities
                    device = outputs.sequences.device
                    scores_list = outputs.scores  # tuple of [B*num_gen, vocab_size]
                    seq_len = len(scores_list)
                    batch_size_total = scores_list[0].shape[0]  # B*num_gen
                    
                    # 获取生成的 token IDs (跳过 decoder_start_token)
                    generated_token_ids = outputs.sequences[:, 1:1+seq_len]  # [B*num_gen, seq_len]
                    
                    # 计算每个位置的 log prob
                    log_probs_list = []
                    for step_idx in range(seq_len):
                        step_log_probs = torch.log_softmax(scores_list[step_idx], dim=-1)  # [B*num_gen, vocab_size]
                        # Gather 对应 token 的 log prob
                        token_ids = generated_token_ids[:, step_idx]  # [B*num_gen]
                    ...
                    # Stack 并计算平均 (length-normalized)
                    token_log_probs_tensor = torch.stack(log_probs_list, dim=0)  # [seq_len, B*num_gen]
                    generated_scores = token_log_probs_tensor.mean(dim=0)  # [B*num_gen]
                else:
                    # 如果都没有，设为 None
                    generated_scores = None
                    print("WARNING: Cannot compute sequences_scores from outputs.scores")
        else:
            generated_ids = None
            generated_scores = None
        ...
        # ===== Prepare sequence scores for reward (CalibrationReward) =====
        # 将 beam search 分数按 [gen_0, ..., gen_{n-1}, gt] 的顺序排列
        if generated_scores is not None:
            scores_reshaped = generated_scores.view(total_batch_size, num_generated)
            if self.add_gt and num_gt_per_sample > 0:
                # GT 的 score 设为 0.0（占位，正样本不使用 score）
                gt_scores = torch.zeros(total_batch_size, num_gt_per_sample, device=device)
                all_scores = torch.cat([scores_reshaped, gt_scores], dim=1)
            else:
                all_scores = scores_reshaped
            all_scores_list = all_scores.view(-1).cpu().tolist()
        else:
        ...
        rewards = self.reward_func(
            ...
            generated_scores=all_scores_list,          # 传递 beam search 序列分数
        )
