...
class RankPOOffPolicyTrainer(BaseOnlineRLTrainer):
    ...
    def _generate_and_score_completions(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        ✅ 优化：
        - 随机采样序列：用于训练，向量化计算 log probabilities
        - Beam search 序列：仅用于 quantile 估计，直接使用 beam scores
        """
        ...
        # ===== 1. Generate sequences with SAMPLING (for training) =====
        if num_generated > 0:
            with torch.no_grad():
                ...
                # 提取采样序列（去掉 decoder_start_token）
                generated_ids = sampling_outputs.sequences[:, 1:]
                generated_ids = torch.cat([
                    generated_ids,
                    torch.full_like(generated_ids[:, :1], self.eos_token_id)
                ], dim=1)
                
                # ✅ 向量化计算采样序列的 log probabilities
                # scores 是 tuple，转换为 tensor: [seq_len, batch_size, vocab_size]
                scores_tensor = torch.stack(sampling_outputs.scores, dim=0)  # [seq_len, B*num_gen, vocab_size]
                
                # 计算 log softmax
                log_probs = torch.log_softmax(scores_tensor, dim=-1)  # [seq_len, B*num_gen, vocab_size]
                
                # 获取生成的 token IDs (跳过 decoder_start_token)
                generated_token_ids = sampling_outputs.sequences[:, 1:1+len(sampling_outputs.scores)]  # [B*num_gen, seq_len]
                generated_token_ids = generated_token_ids.transpose(0, 1)  # [seq_len, B*num_gen]
                
                # 一次性 gather 所有 token 的 log probs
                selected_log_probs = torch.gather(
                    log_probs,  # [seq_len, B*num_gen, vocab_size]
                    dim=2,
                    index=generated_token_ids.unsqueeze(-1)  # [seq_len, B*num_gen, 1]
                ).squeeze(-1)  # [seq_len, B*num_gen]
                
                # 对序列维度求和得到总分数
                generated_scores = selected_log_probs.sum(dim=0)  # [B*num_gen]
        ...
        # ===== 2. Generate sequences with BEAM SEARCH (仅用于 quantile) =====
        if num_generated > 0:
            with torch.no_grad():
                ...
                # ✅ 直接使用 beam search 的 sequences_scores（不重新计算 logits）
                quantile_scores = beam_outputs.sequences_scores
        else:
        ...
        if generated_ids is not None:
            ...
            quantile_scores = quantile_scores.view(total_batch_size, num_generated)
        
        # ===== Compute quantile (使用 beam search 的分数) =====
        if quantile_scores is not None:
            K = num_generated
            quantiles = quantile_scores[:, K-1]  # [B] - 取第 K 个 beam 的分数
        else:
            quantiles = None
