...
def stage2_train_generation_model(
    model_config,
    rqvae_config,
    offline_rl_config: DictConfig,  # 🔥 保持为 DictConfig
    output_dirs,
    accelerator,
    logger,
    force_retrain=False
    ...
    if accelerator.is_main_process:
        logger.info("\n" + "="*60)
        logger.info("阶段2: 训练生成模型 (Post-training with Offline RL)")
        logger.info("="*60)
    
    # ============ 确定模型保存路径 ============
    # 优先使用 offline_rl_config 中指定的路径
    ...
    try:
        ...
        trainer = setup_training(
            ...
            valid_dataset,
            model_config,
            offline_rl_config,  # 直接传递 DictConfig
            output_dirs,
            logger,
