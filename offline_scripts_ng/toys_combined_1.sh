#!/bin/bash
# Toys数据集GRPO Off-Policy后训练脚本 - Combined奖励版本（Prefix + Calibration）
# CombinedReward: 前缀奖励 + 置信度惩罚
# 保存到 offline_combine_v1 文件夹
# 调参: lr, gamma, alpha, lambda_param, penalty_weight（不含 beta）
...
# ========== 路径配置 ==========
BASE_OUTPUT_DIR="/home/notebook/data/group/RLGR/output_toys_ut"
DATA_DIR="./data/Toys"
# 预训练模型路径（使用user_tokens的预训练模型）
PRETRAINED_MODEL="${BASE_OUTPUT_DIR}/generation_model/lr4e-3_wd0.7"

# ========== 调参范围 ==========
# 后训练学习率
LR_LIST=(1e-4)
# CombinedReward 四个参数
GAMMA_LIST=(0.2 0.25 0.3)       # 前缀奖励上界
ALPHA_LIST=(0.3 0.5)               # 前缀幂律形状参数
LAMBDA_LIST=(1.0 1.5)          # Calibration 聚焦参数
PENALTY_WEIGHT_LIST=(0.4)      # 惩罚权重
# 固定：beta=1, num_tokens_per_item=4
grpo_beta=1
num_tokens_per_item=4
...
echo "=============================================="
echo "Toys数据集 GRPO Off-Policy 后训练 (Combined奖励)"
echo "=============================================="
echo "GPU数量: ${NUM_GPUS}"
...
echo "学习率范围: ${LR_LIST[*]}"
echo "gamma范围: ${GAMMA_LIST[*]}"
echo "alpha范围: ${ALPHA_LIST[*]}"
echo "lambda范围: ${LAMBDA_LIST[*]}"
echo "penalty_weight范围: ${PENALTY_WEIGHT_LIST[*]}"
echo "奖励函数: Combined (Prefix + Calibration)"
...
echo "保存路径: ${PRETRAINED_MODEL}/offline_combine_v1/"
echo "=============================================="
echo ""
echo "固定参数："
echo "  - weight_decay: ${model_decay}"
echo "  - beta: ${grpo_beta}"
echo "  - num_tokens_per_item: ${num_tokens_per_item}"
...
# 总实验数
total_exp=$((${#LR_LIST[@]} * ${#GAMMA_LIST[@]} * ${#ALPHA_LIST[@]} * ${#LAMBDA_LIST[@]} * ${#PENALTY_WEIGHT_LIST[@]}))
current_exp=0

# 用于存储所有实验结果的数组
declare -a all_results

# 遍历 lr、gamma、alpha、lambda、penalty_weight
for model_lr in "${LR_LIST[@]}"; do
    for prefix_gamma in "${GAMMA_LIST[@]}"; do
        for prefix_alpha in "${ALPHA_LIST[@]}"; do
            for lambda_param in "${LAMBDA_LIST[@]}"; do
                for penalty_weight in "${PENALTY_WEIGHT_LIST[@]}"; do
                    current_exp=$((current_exp + 1))
                    
                    # 构建实验名称和保存路径（lr + 四参数）
                    exp_name="lr${model_lr}_gamma${prefix_gamma}_alpha${prefix_alpha}_lambda${lambda_param}_pw${penalty_weight}"
                    posttrain_save_path="${PRETRAINED_MODEL}/offline_combine_v1/${exp_name}"
                    ...
                    echo "=============================================="
                    echo "学习率: ${model_lr}"
                    echo "gamma: ${prefix_gamma}, alpha: ${prefix_alpha}"
                    echo "lambda: ${lambda_param}, penalty_weight: ${penalty_weight}"
                    echo "奖励函数: Combined (Prefix + Calibration)"
                    echo "模型保存路径: ${posttrain_save_path}"
                    echo "=============================================="
                    
                    # CombinedReward 配置
                    reward_config="online_rl.trainer.reward_func._target_=genrec.rewards.combined_reward.CombinedReward +online_rl.trainer.reward_func.num_tokens_per_item=${num_tokens_per_item} +online_rl.trainer.reward_func.gamma=${prefix_gamma} +online_rl.trainer.reward_func.alpha=${prefix_alpha} +online_rl.trainer.reward_func.lambda_param=${lambda_param} +online_rl.trainer.reward_func.penalty_weight=${penalty_weight}"
...
if [ -n "$best_exp_name" ]; then
    ...
    echo "----------------------------------------------"
    echo "模型路径: ${PRETRAINED_MODEL}/offline_combine_v1/${best_exp_name}"
else
    echo "⚠️ 没有成功完成的实验"
fi
echo "=============================================="
    ...
    for prefix_gamma in "${GAMMA_LIST[@]}"; do
        for prefix_alpha in "${ALPHA_LIST[@]}"; do
            for lambda_param in "${LAMBDA_LIST[@]}"; do
                for penalty_weight in "${PENALTY_WEIGHT_LIST[@]}"; do
                    exp_name="lr${model_lr}_gamma${prefix_gamma}_alpha${prefix_alpha}_lambda${lambda_param}_pw${penalty_weight}"
                    echo "  - ${PRETRAINED_MODEL}/offline_combine_v1/${exp_name}/"
                done
            done
        done
    done
