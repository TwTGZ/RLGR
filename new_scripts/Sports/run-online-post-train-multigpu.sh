...
model_bsz=512              # 批次大小（总batch，会自动分配到各卡）
online_beta=1              # GRPO的KL散度系数
online_negnum=4            # 每个样本生成的候选数量
num_epochs=1              # 训练轮数（与单卡版本对齐）
num_gpus=2                 # GPU数量
...
POSTTRAIN_SAVE="./output/output_sports_posttrain_multigpu/posttrain_model"
...
echo "批次大小: $model_bsz (总batch，每卡$(($model_bsz/$num_gpus)))"
