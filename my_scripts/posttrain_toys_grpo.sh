#!/bin/bash
# Toys数据集GRPO On-Policy后训练脚本 - 不带user_tokens版本
# 基于预训练模型进行后训练，调节学习率和奖励函数
# 每个实验保存到独立的文件夹，日志也独立
# 训练结束后汇总所有结果并输出最优参数
...
num_epochs=200             # 训练轮数（有早停）
eval_epoch=5               # 每5个epoch评估一次
early_stop=5               # 早停耐心值

# GRPO 特有参数
grpo_beta=1                # KL散度系数（默认）
grpo_num_gen=8             # 每个样本生成候选数量

# ========== 路径配置 ==========
BASE_OUTPUT_DIR="/home/notebook/data/group/RLGR/output_toys"
DATA_DIR="./data/Toys"
# 预训练模型路径（你的最优模型，不带user_tokens）
PRETRAINED_MODEL="${BASE_OUTPUT_DIR}/generation_model/lr4e-3_wd0.8"

# ========== 调参范围 ==========
# 后训练学习率
LR_LIST=(1e-3 5e-3 1e-4 5e-4 1e-5 5e-5)

# 奖励函数配置
# match: MatchReward - 简单二元匹配奖励
# ndcg:  GRPOReward  - 带NDCG排名惩罚的奖励
...
echo "=============================================="
echo "Toys数据集 GRPO On-Policy 后训练"
echo "=============================================="
echo "GPU数量: ${NUM_GPUS}"
...
echo "  - use_user_tokens: False (不使用)"
echo "=============================================="
echo ""
...
for model_lr in "${LR_LIST[@]}"; do
    for reward_type in "${REWARD_LIST[@]}"; do
        ...
        # 构建实验名称和保存路径
        # 后训练模型保存在预训练模型目录下的 posttrain_model 文件夹
        ...
        if [ -f "${posttrain_save_path}/config.json" ]; then
            ...
            # 运行后训练（不使用user_tokens，使用默认值False）
...
for model_lr in "${LR_LIST[@]}"; do
    for reward_type in "${REWARD_LIST[@]}"; do
        exp_name="lr${model_lr}_${reward_type}"
        echo "  - ${PRETRAINED_MODEL}/posttrain_model/${exp_name}/"
    done
done
echo "=============================================="
