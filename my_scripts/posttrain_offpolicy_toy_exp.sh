#!/bin/bash
# Sports数据集GRPO Off-Policy后训练测试脚本 - 玩具版本
# 用于测试后训练代码是否能正常运行，以及文件结构是否正确
# 2组实验（2个reward），1个epoch
...
echo "=============================================="
echo "🧪 测试脚本：验证GRPO Off-Policy后训练框架"
echo "=============================================="
echo "GPU数量: ${NUM_GPUS}"
...
for model_lr in "${LR_LIST[@]}"; do
    for reward_type in "${REWARD_LIST[@]}"; do
        ...
        posttrain_save_path="${PRETRAINED_MODEL}/posttrain_offline/${exp_name}"
        ...
        elif [ "$reward_type" == "prefix" ]; then
            # prefix - PrefixMatchReward with NDCG penalty
            reward_config="online_rl.trainer.reward_func._target_=genrec.rewards.prefix_match_reward.PrefixMatchReward +online_rl.trainer.reward_func.use_ndcg=True +online_rl.trainer.reward_func.ndcg_weight=0.5 +online_rl.trainer.reward_func.num_tokens_per_item=4"
        else
            echo "错误: 未知的奖励函数类型: ${reward_type}"
            continue
        fi
...
if [ -n "$best_exp_name" ]; then
    ...
    echo "----------------------------------------------"
    echo "模型路径: ${PRETRAINED_MODEL}/posttrain_offline/${best_exp_name}"
else
    echo "⚠️ 没有成功完成的实验"
fi
echo "=============================================="
...
echo ""
echo "实际生成的目录："
ls -la "${PRETRAINED_MODEL}/posttrain_offline/" 2>/dev/null || echo "目录不存在"