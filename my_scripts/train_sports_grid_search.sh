#!/bin/bash
# Sports数据集二阶段调参脚本 - 多卡版本
# 遍历 lr 和 weight_decay 的组合进行训练
# 每个实验保存到独立的文件夹，日志也独立
# 训练结束后汇总所有结果并输出最优参数
...
early_stop=5               # 早停耐心值（连续5次评估无提升则停止）

# ========== 路径配置 ==========
BASE_OUTPUT_DIR="/home/notebook/data/group/RLGR/output_sports"
DATA_DIR="./data/Sports"

# ========== 调参范围 ==========
LR_LIST=(1e-3 2e-3 3e-3 4e-3)
WD_LIST=(0.1 0.2 0.3 0.4)
...
echo "=============================================="
echo "Sports数据集二阶段调参训练"
echo "=============================================="
echo "GPU数量: ${NUM_GPUS}"
echo "总批次大小: ${model_bsz} (每卡: $((model_bsz / NUM_GPUS)))"
echo "训练轮数: ${num_epochs}"
echo "评估频率: 每${eval_epoch}个epoch"
echo "早停耐心值: ${early_stop} (即连续$((early_stop * eval_epoch))个epoch无提升则停止)"
echo "学习率范围: ${LR_LIST[*]}"
echo "权重衰减范围: ${WD_LIST[*]}"
