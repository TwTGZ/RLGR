#!/bin/bash
# Sports数据集GRPO Off-Policy后训练脚本 - Prefix奖励版本
# 基于预训练模型进行后训练，调节学习率
# 每个实验保存到独立的文件夹，日志也独立
# 训练结束后汇总所有结果并输出最优参数
...
num_epochs=100             # 训练轮数（有早停）
eval_epoch=5               # 每5个epoch评估一次
early_stop=4               # 早停耐心值

# GRPO Off-Policy 特有参数
grpo_beta=1                # KL散度系数
grpo_num_gen=8             # 每个样本生成候选数量
...
echo "=============================================="
echo "Sports数据集 GRPO Off-Policy 后训练 (Prefix奖励)"
echo "=============================================="
echo "GPU数量: ${NUM_GPUS}"
...
for model_lr in "${LR_LIST[@]}"; do
    ...
    ndcg5=$(grep "NDCG@5:" "$log_file" | grep -v "eval_" | tail -1 | sed 's/.*Hit@5: \([0-9.]*\).*/\1/' | tr -d '\n\r ')
