#!/bin/bash
# Sports数据集GRPO Off-Policy后训练脚本 - Calibration奖励版本
# 基于预训练模型进行后训练，调节学习率
# 每个实验保存到独立的文件夹，日志也独立
# 训练结束后汇总所有结果并输出最优参数
...
echo "=============================================="
echo "Sports数据集 GRPO Off-Policy 后训练 (Calibration奖励)"
echo "=============================================="
echo "GPU数量: ${NUM_GPUS}"
...
echo "学习率范围: ${LR_LIST[*]}"
echo "奖励函数: CalibrationReward (lambda=${lambda_param})"
...
for model_lr in "${LR_LIST[@]}"; do
    ...
    # 构建实验名称和保存路径（只用 lr 命名）
    exp_name="lr${model_lr}"
    ...
    # Calibration 奖励配置
    reward_config="online_rl.trainer.reward_func._target_=genrec.rewards.calibration_reward.CalibrationReward +online_rl.trainer.reward_func.lambda_param=${lambda_param}"
