#!/bin/bash
# Toys数据集GRPO Off-Policy后训练脚本 - NDCG奖励版本（使用user_tokens）
# 基于预训练模型进行后训练，调节学习率
# 每个实验保存到独立的文件夹，日志也独立
# 训练结束后汇总所有结果并输出最优参数
...
echo "=============================================="
echo "Toys数据集 GRPO Off-Policy 后训练 (NDCG奖励)"
echo "=============================================="
echo "GPU数量: ${NUM_GPUS}"
...
for model_lr in "${LR_LIST[@]}"; do
    for grpo_beta in "${BETA_LIST[@]}"; do
        ...
        # 构建实验名称和保存路径（使用 lr 和 beta 组合命名）
        exp_name="lr${model_lr}_beta${grpo_beta}"
        posttrain_save_path="${PRETRAINED_MODEL}/offline_rere_final/${exp_name}"
...
if [ -n "$best_exp_name" ]; then
    ...
    echo "----------------------------------------------"
    echo "模型路径: ${PRETRAINED_MODEL}/offline_rere_final/${best_exp_name}"
else
    echo "⚠️ 没有成功完成的实验"
fi
echo "=============================================="
...
for model_lr in "${LR_LIST[@]}"; do
    for grpo_beta in "${BETA_LIST[@]}"; do
        exp_name="lr${model_lr}_beta${grpo_beta}"
        echo "  - ${PRETRAINED_MODEL}/offline_rere_final/${exp_name}/"
    done
done
echo "=============================================="