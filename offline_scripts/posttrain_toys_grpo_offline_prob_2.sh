#!/bin/bash
# Toys数据集GRPO Off-Policy后训练脚本 - Calibration奖励版本（使用user_tokens）
# 基于预训练模型进行后训练，调节学习率
# 每个实验保存到独立的文件夹，日志也独立
# 训练结束后汇总所有结果并输出最优参数
...
grpo_num_gen=8             # 每个样本生成候选数量（add_gt=True时会自动减1给GT腾位置，最终总共4个）
...
# ========== 调参范围 ==========
# 后训练学习率（与 Beauty 脚本对齐）
LR_LIST=(1e-4 5e-5)
# KL散度系数（与 Beauty 脚本对齐）
BETA_LIST=(1)
# CalibrationReward 特有：置信度聚焦参数 λ
LAMBDA_LIST=(2)
...
echo "=============================================="
echo "Toys数据集 GRPO Off-Policy 后训练 (Calibration奖励)"
echo "=============================================="
echo "GPU数量: ${NUM_GPUS}"
...
echo "Calibration lambda范围: ${LAMBDA_LIST[*]}"
echo "奖励函数: CalibrationReward"
...
# 总实验数
total_exp=$((${#LR_LIST[@]} * ${#BETA_LIST[@]} * ${#LAMBDA_LIST[@]}))
current_exp=0
...
for model_lr in "${LR_LIST[@]}"; do
    for grpo_beta in "${BETA_LIST[@]}"; do
        for grpo_lambda in "${LAMBDA_LIST[@]}"; do
            current_exp=$((current_exp + 1))
            
            # 构建实验名称和保存路径（使用 lr、beta、lambda 组合命名）
            exp_name="lr${model_lr}_beta${grpo_beta}_lambda${grpo_lambda}"
            posttrain_save_path="${PRETRAINED_MODEL}/offline_calibration/${exp_name}"
            ...
            echo "Calibration lambda: ${grpo_lambda}"
            echo "奖励函数: CalibrationReward"
            echo "模型保存路径: ${posttrain_save_path}"
            echo "=============================================="
            
            # Calibration 奖励配置
            reward_config="online_rl.trainer.reward_func._target_=genrec.rewards.calibration_reward.CalibrationReward +online_rl.trainer.reward_func.lambda_param=${grpo_lambda}"
...
if [ -n "$best_exp_name" ]; then
    ...
    echo "----------------------------------------------"
    echo "模型路径: ${PRETRAINED_MODEL}/offline_calibration/${best_exp_name}"
else
    echo "⚠️ 没有成功完成的实验"
fi
echo "=============================================="
...
for model_lr in "${LR_LIST[@]}"; do
    for grpo_beta in "${BETA_LIST[@]}"; do
        for grpo_lambda in "${LAMBDA_LIST[@]}"; do
            exp_name="lr${model_lr}_beta${grpo_beta}_lambda${grpo_lambda}"
            echo "  - ${PRETRAINED_MODEL}/offline_calibration/${exp_name}/"
        done
    done
done
echo "=============================================="
